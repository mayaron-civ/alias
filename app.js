...
const PLAIN = [[1,0,1]];
const RICH  = [[1,0,1],[1,-7,.55],[2,4,.24],[3,-3,.08]];
...
function drawCard(){
  ...
  if(cur.kind === 'word'){
    ...
    if(hint){
      ...
      explain(word);              // тянем заранее, чтобы открылось мгновенно
    }
  }
...
function swipeable(el){
  ...
  el.addEventListener('pointerdown', e=>{
    ...
    id = e.pointerId; x0 = e.clientX; y0 = e.clientY; t0 = Date.now(); dx = 0; live = true;
  ...
  el.addEventListener('pointermove', e=>{
    ...
    el.classList.toggle('sw-ok',  dx > 30);
    el.classList.toggle('sw-bad', dx < -30);
  });
  const finish = e=>{
...
function flyAway(ok){
  ...
  el.style.transform = `translateX(${ok?112:-112}vw) rotate(${ok?18:-18}deg)`;
  return true;
}
